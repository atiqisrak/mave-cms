...
const ImageConverter = () => {
        ...
        },
        {
            ...
            render: (text, record) => (
                <>
                    {record.converted ? (
                        <Button
                            onClick={() => handleDownload(record)} style={{
                        ...
                        <Button loading={loading} style={{
                            ...
                            {loading ?
                                <Spin indicator={<LoadingOutlined style={{ fontSize: 24 }} spin />} /> : 'Convert'}
                        </Button>
                    )}
                </>
            )
    ...
    const handleConvert = (record, conversionType) => {
        ...
        setTimeout(() => {
            const updatedImages = images.map(img =>
                img.key === record.key ? { ...img, converted: true, format: conversionType } : img
            );
            setImages(updatedImages);
            setConvertedImages([...convertedImages, { ...record, format: conversionType }]);
            setLoading(false);
    ...
    const handleDownload = (record) => {
        ...
        const convertedImage = convertedImages.find(img => img.key === record.key);
        if (convertedImage) {
            const blob = new Blob([convertedImage.file], { type: 'image/' + convertedImage.format });
        ...
        <div className="ViewContainer ViewContentContainer">
            ...
            <div style={{ marginTop: 16, display: 'flex', justifyContent: 'space-between' }}>
                ...
                <Button
                    ...
                    onClick={() => message.success('Contact Admin to unlock this feature')}
                >
                    Convert All
                </Button>
            </div>
